...
def get_html(url: str) -> str:
    ...
    if r.apparent_encoding:
        r.encoding = r.apparent_encoding  # často cp1250
    return r.text
...
def parse_municipalities(index_url: str) -> List[Tuple[str, str, str]]:
    """
    Zo stránky okresu (ps32) vytiahne zoznam obcí ako (code, location, detail_url).
    Odkazy na detail obce sú typu 'ps311?...'
    """
...
def extract_summary_and_parties(detail_url: str) -> Tuple[int, int, int, Dict[str, int]]:
    """
    Vráti (registered, envelopes, valid, {party: votes}) pre jednu obec.
    Robí JEDEN HTTP request (HTML posunie do pandas.read_html).
    """
    ...
    for df in tables:
        ...
        if not name_col or not votes_col:
            continue
